...
%%% Type 1 :
 T_UP    = 410 ;  %% Positive z direction
 T_DOWN  = 400 ;  %% Negative z direction
 T_LEFT  = 210 ;  %% Negative r direction
...
% d_New(1:NM*NL,1) = d(:,1);
% d_New(NM*NL+1:NM*NL+2*(NL-1)*(M-2),1) = 0;
...
for i=0:NL-1
 ...
 L_11(:,:) = L_1_G(:,(i*N+1):(i*N+N));
 
 for j=1:N
     A_r_L1(j,:) = L_11(1,j)*A_r_G(j,i*N+1:i*N+N);         
 end
...
%        for j=(i*NM+2*N):N:((i+1)*NM-N)
%            for k=0:N-1
% %                C_T(j,j-k)=C_T(j,j-k)+(GaM)*A_r_G_L2_ext_p(1,N-k);
%                C_T(j,j-k)=A_r_G_L2_ext_p(1,N-k);
%            end 
%        end
%        
%        for j=(i*NM+2*N):N:((i+1)*NM-N)
%            for k=0:N-1
% %                C_T(j,j+k+NM-N+1)=C_T(j,j+k+NM-N+1)+(GaM)*A_r_G_L2_ext_n(1,k+1);
%                C_T(j,j+k+NM-N+1)=A_r_G_L2_ext_n(1,k+1);
%            end 
%        end
       
       for j=(i*NM+2*N):N:((i+1)*NM-N)
%            C_T(j,j)=C_T(j,j)+(GaM)*1-0;
           C_T(j,j)=C_T(j,j)+1-0; 
       end
       
       for j=(i*NM+2*N):N:((i+1)*NM-N)
%            C_T(j,j+NM-N+1)=C_T(j,j+NM-N+1)-(GaM)*1+0;
...
%        for j=(i*NM+N+1):N:((i+1)*NM-2*N+1)
%            for k=0:N-1
% %                C_T(j,j-k-NM+N-1)=C_T(j,j-k-NM+N-1)+(GaM)*A_r_G_L2_ext_p(1,N-k);
...
%        for j=(i*NM+N+1):N:((i+1)*NM-2*N+1)
%            for k=0:N-1
% %                C_T(j,j+k)=C_T(j,j+k)+(GaM)*A_r_G_L2_ext_n(1,k+1);
%                C_T(j,j+k)=A_r_G_L2_ext_n(1,k+1);
%            end 
%        end
...
% %                    C_T_ext((2*i)*(M-2)+q,j-k+(q-1)*N)=A_r_G_L2_ext_p(1,N-k);
%                    C_T_ext((2*i)*(M-2)+q,q*N+1+k)=A_r_G_L2_ext_p(1,N-k);
%                    
% %                    s=s+1;
%                end 
%                
...
%                C_T_ext((2*i)*(M-2)+q,j+(q-1)*N)=C_T(j,j)+1-0; 
%            end
%        end
%        
%        for q=1:M-2
...
CC=zeros(NM*NL);
 ...
 for i=0:NL-1
     ...
     if i==0
       ...
       for j=(i*NM+2*N):N:((i+1)*NM-N)
           for k=0:N-1
               CC(j,j-k)=A_r_G_L2_ext_p(1,N-k);
           end 
       end
       
       for j=(i*NM+2*N):N:((i+1)*NM-N)
           for k=0:N-1
               CC(j,j+k+NM-N+1)=A_r_G_L2_ext_n(1,k+1);
           end 
       end
...
%        
%        for j=(i*NM+2*N):N:((i+1)*NM-N)
%            CC(j,j+NM-N+1)=CC(j,j+NM-N+1)-1+0;
%        end
       
       
       for j=(i*NM+2*N):N:((i+1)*NM-N)
           C_T_ext((2*i)*(M-2)+((j-(i*NM+2*N))/N)+1,:)=CC(j,:);
       end
...
%        for q=1:M-2 
%            for j=(i*NM+2*N):N:((i+1)*NM-N)
%                C_T_ext((2*i)*(M-2)+q,:)=CC(j,:);
%            end
%        end
       
%        II=1;
%        for j=(i*NM+2*N):N:((i+1)*NM-N)
%            JJ(1,II)=((j-(i*NM+2*N))/N)+1;
%            II=II+1;
%        end
     ...
     elseif i==NL-1
       ...
       for j=(i*NM+N+1):N:((i+1)*NM-2*N+1)
           for k=0:N-1
               CC(j,j-k-NM+N-1)=A_r_G_L2_ext_p(1,N-k);
           end 
       end
       
       for j=(i*NM+N+1):N:((i+1)*NM-2*N+1)
           for k=0:N-1
               CC(j,j+k)=A_r_G_L2_ext_n(1,k+1);
           end 
       end
...
%        end
       ...
       for j=(i*NM+N+1):N:((i+1)*NM-2*N+1)
           C_T_ext((2*i-1)*(M-2)+((j-(i*NM+N+1))/N)+1,:)=CC(j,:); 
       end
       
       
%        for j=(i*NM+N+1):N:((i+1)*NM-2*N+1)
%            for q=1:M-2
%                C_T_ext((2*i-1)*(M-2)+q,:)=CC(j,:);
%            end 
%        end
...
% C_T_new=eye(NM*NL+(2*(NL-1)*(M-2)));
% C_T_new=zeros(NM*NL+(2*(NL-1)*(M-2)));

C_T_new(1:NM*NL,1:NM*NL)=C_T(:,:);
C_T_new(NM*NL+1:NM*NL+(2*(NL-1)*(M-2)),1:NM*NL)=C_T_ext(:,:);



d_new(1:NM*NL,:)=d(:,:);
d_new(NM*NL+1:NM*NL+(2*(NL-1)*(M-2)),:)=0;
...
% er_r=0;
% for i=1:NM_G+(2*(NL-1)*(M-2))
%     for j=1:NM_G+(2*(NL-1)*(M-2))
%         if C_T_new(i,1:NM_G+(2*(NL-1)*(M-2))) ==0
%             er_r=er_r+1;
%             i;
%             j;
%         end
...
% for i=1:NM_G+(2*(NL-1)*(M-2))
%     for j=1:NM_G+(2*(NL-1)*(M-2))
%         if C_T_new(1:NM_G+(2*(NL-1)*(M-2)),i) ==0
%             er_c=er_c+1;
%             i;
%             j;
%         end
...
for i=0:NL-1
    ...
    T_new_calc(i*N+1:i*N+N,1:M)=reshape(T_new(i*NM+1:i*NM+NM,1),N,M);
    
end
T_new_calc;
T_new_real = T_new_calc'


%%

C_T_new2=C_T_new'*C_T_new;

T_new2 = C_T_new2 \ d;

for i=0:NL-1
    
    T_new2_calc(i*N+1:i*N+N,1:M)=reshape(T_new2(i*NM+1:i*NM+NM,1),N,M);
    
end
T_new2_real = T_new2_calc'
