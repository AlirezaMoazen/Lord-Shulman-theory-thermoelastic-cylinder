...
models = {'UD','A','V','X','O'};
minColorIntensity = 0.35; % ????? ??? ???
UD_darkFactor = 0.5;     % ????? ???? UD
...
for m = 1:length(models)
    ...
    if strcmp(models{m}, 'UD')
        fracs = minColorIntensity + fracs * (1 - UD_darkFactor);
    else
        fracs = minColorIntensity + fracs * (1 - minColorIntensity);
    end
    ...
    for i = 1:nLayers
        ...
        c = fracs(i); % ??? ???? ?? ?? ????
        fill([X1 X2],[Y1 Y2], [0 0 c] + (1-c)*[1 1 1], 'EdgeColor','k'); % ??? ???? ?????
        hold on;
    end
...
for m = 1:length(models)
    ...
    for i = 1:nLayers
        ...
        X2 = r2*cos(fliplr(theta)); Y2 = r2*sin(fliplr(theta));
        c = fracs(i); % ??? ???? ?? ?? ????
        fill([X1 X2],[Y1 Y2], [c 0 0] + (1-c)*[1 1 1], 'EdgeColor','k'); % ???? ???? ?????
        hold on;
    end
