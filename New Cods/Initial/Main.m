...
%  L        : طول استوانه (متر)
%  r_nodes{e} : بردار نقاط شعاعی لایه e (توزیع چبیشف)
%  z_nodes    : بردار نقاط محوری (چبیشف)
...
L = 0.5;                    % طول استوانه (m)
N_r = 11;                   % تعداد نقاط شعاعی در هر لایه (توصیه میانی 11 تا 15)
N_z = 15;                   % تعداد نقاط محوری

% ضخامت هر لایه (فرض مساوی)
t_layer = (R_o - R_i) / NL;
R_boundaries = linspace(R_i, R_o, NL+1);   % مرزهای بین لایه‌ها

% نقاط محوری با توزیع چبیشف در بازه [0, L]
z_nodes = chebyshev_grid(0, L, N_z);

% نقاط شعاعی در هر لایه (چبیشف در بازه هر لایه)
r_nodes = cell(NL, 1);
for e = 1:NL
...
c_GPL = 710;        % ظرفیت گرمایی ویژه (J/(kg·K))
...
gamma_conn = 1;              % مولفه اتصال (فرض 1)
...
if strcmpi(porosity_pattern, 'V')
    % تعریف تابع با قدر مطلق کسینوس (برای جلوگیری از منفی)
...
for e = 1:NL
    % ---- کسر جرمی GPL با جلوگیری از منفی و بیشتر از 1 ----
    ...
    switch lower(porosity_pattern)
        ...
        case 'o'
            ...
            Factor_E = min(1, Factor_E);   % جلوگیری از افزایش مدول
        case 'x'
            Factor_E = 1 - e2 * (1 - cos(pi * r_rel / l_total));
            Factor_E = min(1, Factor_E);
        case 'v'
            if isnan(e4_sol)
                error('e4_sol محاسبه نشده است. ابتدا الگوی تخلخل V را در بخش 3 فعال کنید.');
            end
            Factor_E = e4_sol * cos(pi * r_rel / (2*l_total) + pi/4);
            Factor_E = max(0, min(1, Factor_E));   % محدودیت فیزیکی
        case 'a'
            if isnan(e5_sol)
                error('e5_sol محاسبه نشده است. ابتدا الگوی تخلخل A را در بخش 3 فعال کنید.');
            end
...
%% ========================= 5. حل معادله حرارتی استاتیک (فوریه) با در نظر گرفتن گرادیان k =========================
Ndof_T = NL * N_r * N_z;
...
for e = 1:NL
    ...
    % محاسبه مشتق k نسبت به r در نقاط لایه با استفاده از A_r
...
% الف) شرط دما در سطح داخلی (r = R_i, e=1, ir=1)
for iz = 1:N_z
    node = idx_T(1, 1, iz);
    K_T(node, :) = 0;
    K_T(node, node) = 1;
...
% ب) شرط همرفت در سطح خارجی (r = R_o, e=NL, ir=N_r)
e_last = NL;
for iz = 1:N_z
...
for e = 1:NL-1
    ...
    for iz = 1:N_z
        % تعریف سطرهای معادله (استفاده از معادلات لایه e در مرز r=Ro)
