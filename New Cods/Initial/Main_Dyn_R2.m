...
for e = 1:NL
    ...
    for ir = 1:N_r
        r = rv(ir); kk = kv(ir);
        % مشتق k در نقطه ir (اسکالر)
        dkdr_ir = sum(A_r{e}(ir,:) * kv(:));
        for iz = 1:N_z
            eq = idx_T(e,ir,iz);
            for jr = 1:N_r
                % جمله k/r * dT/dr + dk/dr * dT/dr + k * d2T/dr2
            ...
            for jr = 1:N_r
                ...
                if ir == jr
                    K(eq_u, idx_U(e,ir,iz)) = K(eq_u, idx_U(e,ir,iz)) ...
                        - (C11 + C22 - C12 + C22) / r^2;
                    % یا معادل: - (2*C11 - C12)/r^2 برای مواد ایزوتروپ
                end
            end
            for jz = 1:N_z
...
for e = 1:NL
    ...
    for ir = 1:N_r
        ...
        for iz = 1:N_z
            ...
            % Optional geometric skip near boundaries
            % keep if you still want to avoid direct boundary coupling
            if (ir == 1) || (ir == N_r) || (iz == 1) || (iz == N_z)
                continue;
            end
...
%% ========================= 7. Apply boundary conditions (روش کلاسیک بدون کاهش DOF) =========================

% همرفت در سطح خارجی (شرط نیومن)
e_last = NL;
for iz = 1:N_z
...
% عایق بودن سطوح انتهایی (dT/dz=0)
for e = 1:NL
    for ir = 1:N_r
...
% --- شرایط مرزی مکانیکی در انتهاها (برای تمام نقاط شعاعی) ---
for e = 1:NL
    for ir = 1:N_r
        r = r_nodes{e}(ir);
        E = E_node{e}(ir); nu = nu_node{e}(ir);
...
% --- پیوستگی مکانیکی بین لایه‌ها (مانند کد اصلی شما) ---
% (این بخش را از کد خودتان بدون تغییر نگه دارید؛ در صورت نبود، همان کد قبلی را قرار دهید)
for e = 1:NL-1
    for iz = 2:N_z-1
...
% --- قید اضافی برای حالت S و F (جلوگیری از حرکت صلب) ---
...
% --- شرایط مرزی شعاعی (فقط برای نقاط میانی در راستای z) ---
% سطح داخلی (r=Ri): sigma_rr = -P_i (در RHS اعمال می‌شود)، tau_rz=0
e=1; ir=1;
for iz = 2:N_z-1
    r = r_nodes{e}(ir);
    E = E_node{e}(ir); nu = nu_node{e}(ir);
...
% سطح خارجی (r=Ro): sigma_rr=0, tau_rz=0
e=NL; ir=N_r;
for iz = 2:N_z-1
    r = r_nodes{e}(ir);
    E = E_node{e}(ir); nu = nu_node{e}(ir);
...
% نقاط پایش (وسط ضخامت، وسط شعاع، وسط طول)
...
for tstep = 1:Nt
    ...
    % بارهای حرارتی: مقادیر دیریکله برای سطح داخلی
    for iz = 1:N_z
        node = idx_T(1,1,iz);
        F_rhs(node) = T_inner;
    end
    % شرط همرفت در سطح خارجی
    for iz = 1:N_z
   ...
   % =========================================================================
    % اصلاح شده: بارهای حرارتی در معادلات مکانیک (ناشی از انبساط حرارتی)
    % =========================================================================
    for e = 1:NL
        for ir = 2:N_r-1 % فیلتر شعاعی: فقط نقاط کاملاً داخلی لایه
            r = r_nodes{e}(ir);
            E = E_node{e}(ir); nu = nu_node{e}(ir);
            C11 = (1-nu)*E/((1+nu)*(1-2*nu));
            C12 = nu*E/((1+nu)*(1-2*nu));
            ...
            for iz = 2:N_z-1 % فیلتر محوری: فقط نقاط کاملاً داخلی طول استوانه
                
                % محاسبه دقیق مشتقات حرارتی با DQ
                dTdr = 0; dTdz = 0;
                for jr = 1:N_r
    ...
    % اعمال مجدد شرایط دیریکله (جلوگیری از خطای تجمعی)
    % for iz = 1:N_z
    %     node = idx_T(1,1,iz);
    %     d(node) = T_inner;
    %     v(node) = 0; a(node) = 0;
    ...
    % end
    % % اصلاح ساده‌تر: تمام گره‌های is_fixed صفر شوند (به جز دمای داخلی که قبلاً مقداردهی شده)
    % for node = find(is_fixed)'
    %     if ~any(node == idx_T(1,1,1:N_z))  % اگر گره دمای داخلی نیست
    %         d(node) = 0; v(node) = 0; a(node) = 0;
    %     end
    % end

    % =========================================================================
    % اصلاح شده: اعمال مجدد شرایط مرزی واقعی برای جلوگیری از خطای عددی تجمعی
    % =========================================================================
    % ۱. تثبیت دمای سطح داخلی
    for iz = 1:N_z
        node = idx_T(1,1,iz);
        d(node) = T_inner;
        v(node) = 0; a(node) = 0;
    ...
    % ۲. صفر کردن گره‌هایی که کاملاً Fixed مکانیکی هستند (مانند تکیه‌گاه گیردار)
    for node = find(is_fixed)'
        % اگر این گره جزو مرزهای دمای داخلی نیست، آن را صفر کن
