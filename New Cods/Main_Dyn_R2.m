...
% --- قید اضا�?ی برای حالت S و F (جلوگیری از حرکت صلب) ---
...
% --- شرایط مرزی شعاعی (�?قط برای نقاط میانی در راستای z) ---
...
for tstep = 1:Nt
   ...
   % =========================================================================
    ...
    % =========================================================================
    for e = 1:NL
        for ir = 2:N_r-1 % �?یلتر شعاعی: �?قط نقاط کاملاً داخلی لایه
            r = r_nodes{e}(ir);
            E = E_node{e}(ir); nu = nu_node{e}(ir);
            C11 = (1-nu)*E/((1+nu)*(1-2*nu));
            C12 = nu*E/((1+nu)*(1-2*nu));
            ...
            for iz = 2:N_z-1 % �?یلتر محوری: �?قط نقاط کاملاً داخلی طول استوانه
    ...
    % end
    % % اصلاح ساده‌تر: تمام گره‌های is_fixed ص�?ر شوند (به جز دمای داخلی که قبلاً مقداردهی شده)
    % for node = find(is_fixed)'
    ...
    % ۲. ص�?ر کردن گره‌هایی که کاملاً Fixed مکانیکی هستند (مانند تکیه‌گاه گیردار)
    for node = find(is_fixed)'
        % اگر این گره جزو مرزهای دمای داخلی نیست، آن را ص�?ر کن
