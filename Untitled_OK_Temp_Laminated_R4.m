...
%         r(1:N)=r_G_bar((i*N+1):(i*N+N)); 
 ...
 for k=0:(M-1)
    A_r_new((k*N+1):(k*N+N),(k*N+1):(k*N+N)) = T_A_r(1:N,1:N);
 end
A_r_new;
...
z=zeros(1,M);
 ...
 for ii=1:M
     z(ii)=0.5*(1-cos((ii-1)*pi/(M-1)))-0.5; %z1(i,1)=z(i); 
 end
 z;
 for ii=1:M
     for jj=1:M
...
%%% Type 1 :
 ...
 T_LEFT  = 200 ;  %% Negative r direction
 T_RIGHT = 200 ;  %% Positive r direction
...
% C_T_1 = A_r_G_new_L1;
 ...
 r_inv(:,:) = (1/ka(1,i+1))*L_1_G(:,(i*N+1):(i*N+N));
 
 for j=1:N
     A_r_invR(j,:) = r_inv(1,j)*A_r_G(j,i*N+1:i*N+N);         
 end
 
 
 for k=0:(M-1)
     A_r_new_invR((k*N+1):(k*N+N),(k*N+1):(k*N+N)) = A_r_invR(1:N,1:N);
 end
 A_r_new_invR;

 
 
 A_r_G_new_invR((i*NM+1):(i*NM+NM),(i*NM+1):(i*NM+NM)) = A_r_new_invR(1:NM,1:NM);
 ...
 r_inv2=diag(1./r_G);
%  A_r_G_invR = r_inv*A_r_G;
 
 
% C_T = C_T_1+C_T_2+C_T_3+C_T_4; 
% C_T = C_T_2+C_T_3+C_T_4; 
% C_T = B_r_G_new+B_z_G_new+A_r_G_new+A_z_G_new;
% C_T = B_r_G_new+B_z_G_new+A_r_G_new;
% C_T = B_r_G_new+B_z_G_new+A_r_G_new_invR;
C_T = B_r_G_new+B_z_G_new;
 ...
 for i=0:NL-1
     ...
     if i==0
       ...
       %%%--- Positive r direction --> i & i+1
        ...
        C_T((i*NM+2*N):N:((i+1)*NM-N),:)=0;
%         C_T((i*NM+2*N):N:((i+1)*NM-N),:)=B_r_G_new((i*NM+2*N):N:((i+1)*NM-N),:)+B_z_G_new((i*NM+2*N):N:((i+1)*NM-N),:);
        A_r_G_L2_ext_p=L_2(1,i+1)*A_r_G(N,(i*N+1:(i+1)*N));
        A_r_G_L2_ext_n=-L_2(1,i+2)*A_r_G(1,((i+1)*N+1:(i+2)*N));
         
       for j=(i*NM+2*N):N:((i+1)*NM-N)
           for k=0:N-1
%                C_T(j,j-k)=C_T(j,j-k)+A_r_G_L2_ext_p(1,N-k);
               C_T(j,j-k)=A_r_G_L2_ext_p(1,N-k);
           end 
       end
       ...
       for j=(i*NM+2*N):N:((i+1)*NM-N)
           for k=0:N-1
%                C_T(j,j+k+NM-N+1)=C_T(j,j+k+NM-N+1)+A_r_G_L2_ext_n(1,k+1);
               C_T(j,j+k+NM-N+1)=A_r_G_L2_ext_n(1,k+1);
           end 
       end
     ...
     elseif i==NL-1
       ...
       C_T((i*NM+N+1):N:((i+1)*NM-2*N+1),:)=0;
%        C_T((i*NM+N+1):N:((i+1)*NM-2*N+1),:)=B_r_G_new((i*NM+N+1):N:((i+1)*NM-2*N+1),:)+B_z_G_new((i*NM+N+1):N:((i+1)*NM-2*N+1),:);
       A_r_G_L2_ext_p=-L_2(1,i)*A_r_G(N,((i-1)*N+1:(i)*N));
       A_r_G_L2_ext_n=L_2(1,i+1)*A_r_G(1,((i)*N+1:(i+1)*N));
       
       for j=(i*NM+N+1):N:((i+1)*NM-2*N+1)
           for k=0:N-1
%                C_T(j,j-k-NM+N-1)=C_T(j,j-k-NM+N-1)+A_r_G_L2_ext_p(1,N-k);
               C_T(j,j-k-NM+N-1)=A_r_G_L2_ext_p(1,N-k);
           end 
       end
       ...
       for j=(i*NM+N+1):N:((i+1)*NM-2*N+1)
           for k=0:N-1
%                C_T(j,j+k)=C_T(j,j+k)+A_r_G_L2_ext_n(1,k+1);
               C_T(j,j+k)=A_r_G_L2_ext_n(1,k+1);
           end 
       end
...
%         
%         for j=(i*NM+2*N):N:((i+1)*NM-N)
%            C_T(j,:)=ka(1,i+1)*A_r_G_new_invR(j,:)-ka(1,i+2)*A_r_G_new_invR(j+NM-N+1,:);
%         end
%        
% %         A_r_G_L2_ext_p=L_2(1,i+1)*A_r_G(N,(i*N+1:(i+1)*N));
% %         A_r_G_L2_ext_n=-L_2(1,i+2)*A_r_G(1,((i+1)*N+1:(i+2)*N));
...
%        
%        for j=(i*NM+N+1):N:((i+1)*NM-2*N+1)
%            C_T(j,:)=ka(1,i)*A_r_G_new_invR(j,:)-ka(1,i+1)*A_r_G_new_invR(j-NM+N-1,:);
%         end
%        
% %        A_r_G_L2_ext_p=-L_2(1,i)*A_r_G(N,((i-1)*N+1:(i)*N));
% %        A_r_G_L2_ext_n=L_2(1,i+1)*A_r_G(1,((i)*N+1:(i+1)*N));
...
% %%% B: ????? ????? ???? ??? ???? (Dirichlet)
% isDirichlet = false(NM*NL,1);  % ????? ???? ???? ??????? ?????? Dirichlet
% 
% for i=0:NL-1
%     
%     if i==0
...
%     if i==0
%         %--- Positive r direction (i & i+1)
%         indices = (i*NM+2*N):N:((i+1)*NM-N);
%         for j = indices
%             if isDirichlet(j), continue; end
%             % ????? ??? i
%             A_r_G_L2_ext_p = L_2(1,i+1)*A_r_G(N,(i*N+1:(i+1)*N));
%             for k=0:N-1
%                 C_T(j,j-k) = A_r_G_L2_ext_p(1,N-k);
...
%             % ????? ?????
%             C_T(j,j) = C_T(j,j)+1;
%             C_T(j,j+NM-N+1) = C_T(j,j+NM-N+1)-1;
%         end
%         
%     elseif i==NL-1
%         %--- Negative r direction (i-1 & i)
%         indices = (i*NM+N+1):N:((i+1)*NM-2*N+1);
%         for j = indices
%             if isDirichlet(j), continue; end
%             A_r_G_L2_ext_p = -L_2(1,i)*A_r_G(N,((i-1)*N+1:(i)*N));
%             A_r_G_L2_ext_n = L_2(1,i+1)*A_r_G(1,((i)*N+1:(i+1)*N));
...
% %%% B: ????? ????? ??? Dirichlet (??? ????)
% 
% Dirichlet_rows = false(NM*NL,1); % ?????? ???? ???? ??????????? ?????? Dirichlet
% 
% for i = 0:NL-1
%     
%     if i == 0
...
%             idx1 = j-k;
%             idx2 = j+k+NM-N+1;
%             if idx1>=1 && idx1<=NM*NL
%                 C_T(j,idx1) = A_r_G_L2_ext_p(1,N-k);
%             end
%             if idx2>=1 && idx2<=NM*NL
%                 C_T(j,idx2) = A_r_G_L2_ext_n(1,k+1);
%             end
%         end
%         
%         % ??? ?? 1 ???? ??????? ????
%         idx_diag = j;
%         idx_diag2 = j+NM-N+1;
%         if idx_diag<=NM*NL && ~Dirichlet_rows(idx_diag)
%             C_T(j,idx_diag) = C_T(j,idx_diag)+1;
%         end
%         if idx_diag2<=NM*NL && ~Dirichlet_rows(idx_diag2)
%             C_T(j,idx_diag2) = C_T(j,idx_diag2)-1;
%         end
%     end
%     
%     % Negative r direction (i-1 & i)
...
%             idx_diag1 = j-NM+N-1;
%             idx_diag2 = j;
%             if idx_diag1>=1 && idx_diag1<=NM*NL && ~Dirichlet_rows(idx_diag1)
%                 C_T(j,idx_diag1) = C_T(j,idx_diag1)+1;
%             end
%             if idx_diag2>=1 && idx_diag2<=NM*NL && ~Dirichlet_rows(idx_diag2)
%                 C_T(j,idx_diag2) = C_T(j,idx_diag2)-1;
%             end
%         end
%     end
...
% %%% ?????????? B ? C
% 
% Dirichlet_rows = false(NM*NL,1);
% 
% %--------- ??? B: ????? ????? ??? Dirichlet ----------
% % Negative z (?????? ???????)
% Dirichlet_rows(1:N) = true;
% C_T(1:N,:) = 0; C_T(1:N,1:N) = eye(N);
% 
% % Positive z (?????? ?? ????)
% Dirichlet_rows(NM-N+1:NM) = true;
% C_T(NM-N+1:NM,:) = 0; C_T(NM-N+1:NM, NM-N+1:NM) = eye(N);
% 
% % Negative r (???? ???????? ?????)
% neg_r_idx = N+1:N:(NM-2*N+1);
% Dirichlet_rows(neg_r_idx) = true;
% C_T(neg_r_idx,:) = 0; 
% for j = 1:length(neg_r_idx)
%     C_T(neg_r_idx(j),neg_r_idx(j)) = 1;
% end
% 
% % Positive r (????? ???? ???)
% pos_r_idx = (NL-1)*NM + 2*N:N:NL*NM-N;
% Dirichlet_rows(pos_r_idx) = true;
% C_T(pos_r_idx,:) = 0; 
% for j = 1:length(pos_r_idx)
%     C_T(pos_r_idx(j),pos_r_idx(j)) = 1;
% end
% 
% %--------- ??? C: ????? ??? ??????? ----------
% for i = 0:NL-1
%     % Positive r direction
%     j_idx = i*NM+2*N:N:(i+1)*NM-N;
%     j_idx = j_idx(~Dirichlet_rows(j_idx));
%     
%     if ~isempty(j_idx)
%         A_p = L_2(1,i+1)*A_r_G(N,(i*N+1:(i+1)*N));
%         A_n = -L_2(1,i+2)*A_r_G(1,((i+1)*N+1:(i+2)*N));
%         for jj = 1:length(j_idx)
%             j = j_idx(jj);
%             C_T(j,j-(0:N-1)) = A_p;
%             C_T(j,j+NM-N+1+(0:N-1)) = A_n;
%             % ??? ?? 1 ? -1 ???? ???????
%             if j <= NM*NL
%                 C_T(j,j) = C_T(j,j)+1;
%                 if j+NM-N+1 <= NM*NL
...
%     % Negative r direction
%     if i>0
%         j_idx = i*NM+N+1:N:(i+1)*NM-2*N+1;
%         j_idx = j_idx(~Dirichlet_rows(j_idx));
%         if ~isempty(j_idx)
%             A_p = -L_2(1,i)*A_r_G(N,((i-1)*N+1:i*N));
%             A_n = L_2(1,i+1)*A_r_G(1,(i*N+1:(i+1)*N));
%             for jj = 1:length(j_idx)
%                 j = j_idx(jj);
%                 C_T(j,j-(NM-N+1)+(0:N-1)) = A_p;
%                 C_T(j,j+(0:N-1)) = A_n;
%                 % ??? ?? 1 ? -1
%                 if j-(NM-N+1) >=1
%                     C_T(j,j-(NM-N+1)) = C_T(j,j-(NM-N+1)) + 1;
...
% 
% %--------- ??? B: ????? ??? Dirichlet ----------
% Dirichlet_rows(1:N) = true;                        % Negative z
% Dirichlet_rows(NM-N+1:NM) = true;                  % Positive z
% Dirichlet_rows(N+1:N:(NM-2*N+1)) = true;          % Negative r
% Dirichlet_rows((NL-1)*NM+2*N:N:NL*NM-N) = true;   % Positive r
% 
% % ????? ?????? ???? ?? ?????? Dirichlet
% C_T(Dirichlet_rows,:) = 0;
% idx = find(Dirichlet_rows);
% for k = 1:length(idx)
%     C_T(idx(k),idx(k)) = 1;
% end
% 
...
for i=0:NL-1
    ...
    D_calc(i*N+1:i*N+N,1:M)=reshape(D(i*NM+1:i*NM+NM,1),N,M);
