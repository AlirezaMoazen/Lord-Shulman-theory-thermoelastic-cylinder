...
| `CHAPTER5_conclusions_FA/EN` (+ docx) | **DONE (R1); FA re-aligned to R4.4, EN NOT** | conclusions + new §5-4 "Limitations of the study" (2026-08-21, both languages, future-work renumbered to §5-5); FA numbers re-aligned to R4.4 (overshoot/clamping/aspect/Gaussian framing corrected) after the `chapter_stats.csv` regeneration fix (§7) — **⚠️ found 2026-08-21: `CHAPTER5_conclusions_EN.md`'s numeric findings (§5-2) are still the old pre-R4.4 set (overshoot 1.41/1.78, GPL-V ⁠−16%/5.5×, 4×4 matrix, 9×/50× headline figures) despite this row's own earlier claim of a refresh — EN and FA now disagree; not fixed in this pass, see `DECISIONS_NEEDED.md`** |
