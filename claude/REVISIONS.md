...
|---|---|---|
| `CHAPTER1_framework_FA_R1` / `CHAPTER2_litreview_FA_R1` / `CHAPTER3_formulation_FA_R1` (+ docx) | **DONE (R1)** | drafted, citation-cleaned; Ch3 Table 3-1 matrix corrected epoxy (was mislabeled metal); Ch3's 5 schematic figures embedded 2026-08-05 (geometry, GPL patterns, porosity, DQM nodes, flowchart) — porosity schematic (Fig 3-3) re-plotted 2026-08-06 to show the actual porosity coefficient e=P_m² per thesis eq. 3-39, not the mass factor P_m it originally (wrongly) plotted; DQM-nodes schematic uses N=9 illustrative points, no titles/floating text (author's plotting rules, §7). Colour track-changes versions exist for all three (`_R1_tracked`). **Fixed 2026-08-22 (two bugs):** (1) the tracked `.docx` files never actually showed color — pandoc silently drops raw-HTML `<span style="color:...">` on docx output, so every "addition" rendered as plain black text; converted all 3 tracked `.md` files to pandoc's native `[text]{custom-style="TrackAdd"/"TrackDel"}` span syntax and rebuilt the reference-doc with real `TrackAdd`/`TrackDel` character styles (green / red+strike) — verified color survives in the actual docx XML this time. (2) all 3 tracked docs had drifted from their current final chapters (found while investigating #1): Ch1 was missing a citation, Ch2 had two swapped reference numbers ([5]/[6]/[78] mixed up), Ch3's tracked copy predated the material-properties table (3-1), the γ=1/2/percolation sentence, and the 5 embedded figures (still had `[شکل: …]` placeholders) — all resynced to match the current final chapters. |
