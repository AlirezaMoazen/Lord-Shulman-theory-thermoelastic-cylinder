%% claude_catalog_R5.m — QUANTITY CATALOG for choosing chapter-4 figures (B&W)
%  REVISION 5 of the catalog. R5 computes eps_zz (axial strain) from the SAVED
%  final-time STRESSES via Hooke's law, instead of the last-snapshot dw/dz used
%  in R3/R4. The solver builds every saved stress from sigma = C*(eps - alpha*dT)
%  with isotropic per-layer stiffness (C13=C12, C33=C11). Subtracting the axial
%  row from the radial row makes the thermal term cancel:
%      sigma_rr - sigma_zz = 2*mu*(eps_rr - eps_zz),
%  and the per-layer shear modulus is read from the same data by
%      sigma_rr - sigma_tt = 2*mu*(eps_rr - eps_tt).
%  Hence  eps_zz = eps_rr - (sigma_rr - sigma_zz)/(2*mu)  is EXACT at the final
%  time for every case (2*mu is taken from each case's own stresses, so it is
%  correct for any weight/porosity/pattern, not just the UD reference).
%  eps_rr = du/dr and eps_thth = u/r stay kinematic (already exact-final).
%  --- heritage ---
%  R1 = frozen 9-panel; R2 = two-part + smoothed; R3 = added the 3 strain panels
%  (eps_zz then from the last snapshot); R4 = D2 study swapped to low-W 0.1-1.5%.
...
%  Curves smoothed by makima densification.
...
studies = { ...
 ...
 'D2_GPL_fill',      {'D3_W_001','D3_W_003','D3_W_005','D3_W_009','D3_W_015'}, {'W=0.1%','W=0.3%','W=0.5%','W=0.9%','W=1.5%'};
...
for si = 1:size(studies,1)
    ...
    fin(gca,FNT,FSZ,'\xi','\epsilon_{zz}','(4) \epsilon_{zz}(\xi) axial [Hooke, exact]');
    ...
    fprintf('catalog R5: %s (hist + prof, Hooke eps_zz)\n', sname);
end
...
function [err,ett,ezz] = strainprof(d)
    ...
    % eps_zz is recovered from the SAVED final-time stresses via Hooke's law
    % (isotropic, C13=C12 & C33=C11):  sig_rr - sig_zz = 2*mu*(eps_rr - eps_zz),
    % with the per-layer shear modulus read from the same data by
    % sig_rr - sig_tt = 2*mu*(eps_rr - eps_tt). The thermal term cancels, so
    % eps_zz is EXACT at the final time (no snapshot). 2*mu comes from each
    % case's own stresses, hence correct for any weight/porosity/pattern.
    NL=d.NL; Nr=d.N_r;
    ett = d.U_all(:) ./ d.r_all(:);
    ...
    for e=1:NL
        ...
        de = ei - ti;  ds = sr - st;  den = de.'*de;   % 2*mu = d(sig)/d(eps) deviatoric
        if den > 1e-30
            twomu = (de.'*ds)/den;                     % per-layer LSQ through origin
        else
            dg = err(:)-ett(:);  twomu = (dg.'*(Srr(:)-Stt(:)))/(dg.'*dg);  % global fallback
        end
        % average the radial and hoop constitutive rows (both exact) for eps_zz:
