%% claude_catalog_R5_color.m — QUANTITY CATALOG (COLOR), revision 5
%  Color companion to claude_catalog_R5.m.
%  REVISION 5 computes eps_zz (axial strain) from the SAVED final-time STRESSES
%  via Hooke's law, instead of the last-snapshot dw/dz used in R3/R4. The solver
%  builds every saved stress from sigma = C*(eps - alpha*dT) with isotropic
%  per-layer stiffness (C13=C12, C33=C11); subtracting the axial row from the
%  radial row cancels the thermal term:
%      sigma_rr - sigma_zz = 2*mu*(eps_rr - eps_zz),
%  and the per-layer 2*mu is read from the same data by
%      sigma_rr - sigma_tt = 2*mu*(eps_rr - eps_tt).
%  Hence eps_zz = eps_rr - (sigma_rr - sigma_zz)/(2*mu) is EXACT at final time
%  for every case. eps_rr = du/dr and eps_thth = u/r stay kinematic (exact).
%  --- heritage ---
%  R1 = frozen 9-panel; R2 = two-part + smoothed; R3 = added 3 strain panels
%  (eps_zz then from last snapshot); R4 = D2 study swapped to low-W 0.1-1.5%.
...
STY.lw = [1.5 1.5 1.5 1.5 1.5];  FNT='Times New Roman'; FSZ=9;
