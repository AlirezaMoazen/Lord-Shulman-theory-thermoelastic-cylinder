%% ========================================================================
%  claude_param_figures_R2.m — thesis figures from the parametric campaign
%  ------------------------------------------------------------------------
%  Revision of claude_param_figures.m (R1 kept frozen). Only change:
%  output folders are now SPLIT for the results chapter —
%    campaign studies (A..N)      -> results_campaign\figures\
%    extension studies (T3_...)   -> results_extensions\figures\
%  Raw case .mat files stay in param_studies\ (data store, unchanged).
%
...
pdir = 'param_studies';
fdir_camp = fullfile('results_campaign','figures');
fdir_ext  = fullfile('results_extensions','figures');
...
for si = 1:size(studies,1)
    ...
    fprintf('figure written: %s -> %s\n', sname, fdir);
    close(fig);
end
fprintf('\nCampaign figures in %s ; extension figures in %s\n', fdir_camp, fdir_ext);
