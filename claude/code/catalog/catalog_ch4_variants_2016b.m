%% claude_catalog_ch4_variants_2016b.m — fill the missing style variants for the Ch4 docx.
%  MATLAB R2016b-COMPATIBLE VARIANT of claude_catalog_ch4_variants.m. Identical
%  output (same figures, same data) — the only unavailable-before-R2018b
%  function, yline(y,...), is replaced with plot(xlim,[y y],...), preserving
%  the same dash-dot/dotted B&W line styles. Do not edit
%  claude_catalog_ch4_variants.m to match this file or vice versa — keep
...
for li=1:numel(L)
    ...
    plot(xlim,[1 1],'-.'); plot(xlim,[0.1 0.1],':'); fin(gca,L(li).xl,'rel. error (%)','(c) convergence');
