%% claude_catalog_conv_2016b.m — CONVERGENCE / MINIMUM-NODES figures (Prom.3 #4)
%  MATLAB R2016b-COMPATIBLE VARIANT of claude_catalog_conv.m. Identical output
%  (same figures, same data) — the only unavailable-before-R2018b function,
%  yline(y,...), is replaced with plot(xlim,[y y],...). Do not edit
%  claude_catalog_conv.m to match this file or vice versa — keep both in
%  sync by hand if the figures change (no-rewrite rule).
%  ------------------------------------------------------------------------
...
for li=1:numel(L)
    ...
    plot(xlim,[1 1],'-','Color',[0.3 0.3 0.3]); plot(xlim,[0.1 0.1],'-','Color',[0.7 0.7 0.7]); fin(gca,L(li).xl,'rel. error vs finest (%)','(c) convergence');
...
set(gca,'YScale','log'); plot(xlim,[1 1],'-','Color',[0.3 0.3 0.3]); plot(xlim,[0.1 0.1],'-','Color',[0.7 0.7 0.7]);
