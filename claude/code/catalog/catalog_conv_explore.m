%% claude_catalog_conv_explore.m — EXPLORATORY convergence / master figure
%  Author request (2026-08-24): a conv_master figure at a NEW anchor point
%  N_r=11, N_z=13, N_L=7 (NOT the locked reference mesh N_r=15/N_z=11 —
%  that stays unchanged for the Ch4 campaign; this is a standalone
%  diagnostic study). Same structure/quantities as claude_catalog_conv.m
%  (Prom.3 #4), just pointed at param_studies_conv_explore/ and the new
%  anchor/case names from campaign/run_conv_explore.ps1. No dt ladder here
%  (not requested). New revision file; reads param_studies_conv_explore;
%  modifies nothing else (claude_catalog_conv.m / figures_ch4 untouched).
