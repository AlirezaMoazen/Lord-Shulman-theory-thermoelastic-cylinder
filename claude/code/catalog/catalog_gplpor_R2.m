%% catalog_gplpor_R2.m — GPL-pattern comparison at each fixed porosity pattern
...
%  (H_<GPL>_<Por>.mat) by porosity, so each figure answers "at this
%  porosity background, how do the GPL distributions compare?".
%  Same 4-panel format & Heydarpour convention as catalog_R6:
%    (1) T*(Fo)  (2) U*(Fo)   [mid-point histories]
%    (3) T*(xi)  (4) Sig_thth(xi)   [profiles at final time]
%  Title-free; legends kept. Renders B&W (-> figures_ch4_bw) and colour
%  (-> figures_ch4_color).
...
CO.ls={'-','-','-','-','-'}; CO.mk={'o','s','^','d','v'}; CO.lw=[1.6 1.4 1.4 1.4 1.4];   % solid colour lines
