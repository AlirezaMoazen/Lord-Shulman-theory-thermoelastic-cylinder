%% ========================================================================
%  claude_ch123_figs_R4.m  —  SCHEMATIC FIGURES FOR CHAPTERS 2-3 (R4)
%  Revision of claude_ch123_figs_R3.m (kept frozen). Author request
%  (2026-08-29): in the porosity V/A pattern formulas (Pm.V, Pm.A), leading
%  coefficient 2 -> 1; denominator inside cos(...) kept at 4 (unchanged
%  from R3), i.e. cos(pi*zet/4 + pi/4) stays, only the 2* prefactor drops
%  to 1*. Per the author's revision-number rule, output saved as a NEW
%  file (porosity_patterns_schematic_R4.png) alongside R1's original
%  (porosity_patterns_schematic.png), R2's (_R2.png) and R3's (_R3.png)
...
Pm.V  = 1*em4*cos(pi*zet/4 + pi/4);                   % max at INNER face
Pm.A  = 1*em5*cos(pi*zet/4 - pi/4);                   % max at OUTER face
...
exportgraphics(fig,fullfile(outdir,'porosity_patterns_schematic_R4.png'),'Resolution',300);
close(fig); fprintf('done: porosity_patterns_schematic_R4.png (R4 output -- R1/R2/R3 porosity PNGs kept frozen/untouched)\n');
