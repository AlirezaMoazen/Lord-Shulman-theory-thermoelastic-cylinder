%% ch3_gpl_porosity_stacked_R2.m — GPL + porosity pattern schematics, stacked
%  Combines the GPL weight-fraction schematic and the porosity-coefficient
%  schematic (both also produced by ch123_figs.m) into ONE figure,
%  stacked vertically (top = GPL, bottom = porosity) rather than side by
%  side -- the two have different x-axes (discrete layer index vs
%  continuous xi), so they are two stacked panels, not a shared-axis overlay.
%  Same data/formulas/styling as ch123_figs.m (ground-truth: frozen
%  solver LSTE_solver_R7.m).
