%% ========================================================================
%  claude_ch123_figs_R3.m  —  SCHEMATIC FIGURES FOR CHAPTERS 2-3 (R3)
%  Revision of claude_ch123_figs_R2.m (kept frozen). Author request
%  (2026-08-27): in the porosity V/A pattern formulas (Pm.V, Pm.A) --
%  (1) leading coefficient reverted 1.5 -> 2; (2) the first fraction's
%  denominator inside cos(...) changed 2 -> 4, i.e. cos(pi*zet/2 + pi/4)
%  -> cos(pi*zet/4 + pi/4) (and the A pattern's minus-sign counterpart).
%  Per the author's revision-number rule, output saved as a NEW file
%  (porosity_patterns_schematic_R3.png) alongside R1's original
%  (porosity_patterns_schematic.png) and R2's (porosity_patterns_schematic_R2.png)
%  -- none overwritten. The other 5 schematics still write to their
...
Pm.V  = 2*em4*cos(pi*zet/4 + pi/4);                   % max at INNER face
Pm.A  = 2*em5*cos(pi*zet/4 - pi/4);                   % max at OUTER face
...
exportgraphics(fig,fullfile(outdir,'porosity_patterns_schematic_R3.png'),'Resolution',300);
close(fig); fprintf('done: porosity_patterns_schematic_R3.png (R3 output -- R1/R2 porosity PNGs kept frozen/untouched)\n');
