%% ========================================================================
%  claude_ch123_figs_R5.m  —  SCHEMATIC FIGURES FOR CHAPTERS 2-3 (R5)
%  Revision of claude_ch123_figs_R4.m (kept frozen). Author request
%  (2026-08-29): in the porosity V/A pattern formulas (Pm.V, Pm.A), leading
%  coefficient stays at 1 (unchanged from R4); denominator inside cos(...)
%  reverted 4 -> 2, i.e. cos(pi*zet/4 + pi/4) -> cos(pi*zet/2 + pi/4).
%  Per the author's revision-number rule, output saved as a NEW file
%  (porosity_patterns_schematic_R5.png) alongside R1's original
%  (porosity_patterns_schematic.png), R2's (_R2.png), R3's (_R3.png) and
%  R4's (_R4.png) -- none overwritten. The other 5 schematics still write
%  to their original filenames since their content is unchanged by this edit.
%  ------------------------------------------------------------------------
...
exportgraphics(fig,fullfile(outdir,'GPL_patterns_schematic.png'),'Resolution',300);
...
Pm.V  = 1*em4*cos(pi*zet/2 + pi/4);                   % max at INNER face
Pm.A  = 1*em5*cos(pi*zet/2 - pi/4);                   % max at OUTER face
...
exportgraphics(fig,fullfile(outdir,'porosity_patterns_schematic_R5.png'),'Resolution',300);
close(fig); fprintf('done: porosity_patterns_schematic_R5.png (R5 output -- R1/R2/R3/R4 porosity PNGs kept frozen/untouched)\n');
...
exportgraphics(fig,fullfile(outdir,'solution_flowchart.png'),'Resolution',300);
...
exportgraphics(fig,fullfile(outdir,'theory_taxonomy.png'),'Resolution',300);
