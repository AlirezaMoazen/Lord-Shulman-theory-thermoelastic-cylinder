%% ========================================================================
%  claude_ch123_figs_R6.m  —  SCHEMATIC FIGURES FOR CHAPTERS 2-3 (R6)
%  Revision of claude_ch123_figs_R4.m (kept frozen). Author request
%  (2026-08-31): shift Pm.V/Pm.A up by a constant +0.5 (R4's coefficient=1,
%  denominator=4 formula unchanged otherwise). Per the author's
%  revision-number rule, output saved as a NEW file
%  (porosity_patterns_schematic_R6.png) alongside R1-R5 -- none
...
Pm.V  = 1*em4*cos(pi*zet/4 + pi/4) + 0.5;             % max at INNER face
Pm.A  = 1*em5*cos(pi*zet/4 - pi/4) + 0.5;             % max at OUTER face
...
exportgraphics(fig,fullfile(outdir,'porosity_patterns_schematic_R6.png'),'Resolution',300);
close(fig); fprintf('done: porosity_patterns_schematic_R6.png (R6 output -- R1-R5 porosity PNGs kept frozen/untouched)\n');
