%% ========================================================================
%  claude_ch123_figs_R7.m  —  SCHEMATIC FIGURES FOR CHAPTERS 2-3 (R7)
%  Revision of claude_ch123_figs_R5.m (kept frozen). Author request
%  (2026-08-31): shift Pm.V/Pm.A up by a constant +0.5 (R5's coefficient=1,
%  denominator=2 formula unchanged otherwise). Per the author's
%  revision-number rule, output saved as a NEW file
%  (porosity_patterns_schematic_R7.png) alongside R1-R6 -- none
%  overwritten. The other 5 schematics still write to their original
%  filenames since their content is unchanged by this edit.
%  ------------------------------------------------------------------------
...
Pm.V  = 1*em4*cos(pi*zet/2 + pi/4) + 0.5;             % max at INNER face
Pm.A  = 1*em5*cos(pi*zet/2 - pi/4) + 0.5;             % max at OUTER face
...
exportgraphics(fig,fullfile(outdir,'porosity_patterns_schematic_R7.png'),'Resolution',300);
close(fig); fprintf('done: porosity_patterns_schematic_R7.png (R7 output -- R1-R6 porosity PNGs kept frozen/untouched)\n');
