%% claude_pattern_gradient_schematics.m — GPL & porosity patterns as gradient-shaded layer wedges
%  Author request (2026-08-29): instead of the line/curve plots
%  (GPL_patterns_schematic.png, porosity_patterns_schematic.png), show the
%  five GPL and five porosity distribution patterns as a GRADIENT — a
%  quarter-annulus divided into the 7 concentric layer bands, each band
%  shaded by its actual intensity, darkest = most graphene / most porous,
%  lightest = least. Style matches the convention in Rezaei's MSc thesis
%  (same university, same UD/X/O/V pattern family; Fig. 2-3, p.17) and the
%  cited FGM literature, extended here to all 5 patterns (UD/O/X/V/A) used
%  in this thesis. Two outputs, no curves/axes-plots:
%     GPL_pattern_gradient.png       (blue scale, by W_GPL^(e))
%     porosity_pattern_gradient.png  (gray scale, by e^(e) = P_m^(e))^2)
...
%  line-plot schematics. New file; does not touch claude_ch123_figs*.m or
%  their PNGs.
...
Pm.V  = 2*em4*cos(pi*zlay/2 + pi/4);
Pm.A  = 2*em5*cos(pi*zlay/2 - pi/4);
Pf.UD = Pm.UD.^2; Pf.O = Pm.O.^2; Pf.X = Pm.X.^2; Pf.V = Pm.V.^2; Pf.A = Pm.A.^2;

%% ---- draw both figures ----
draw_gradient_set(Wg, pat, Rb, NL, [1 1 1], [0.03 0.19 0.42], ...
    'W_{GPL}^{(e)}  (weight fraction)', 'GPL_pattern_gradient.png', outdir);
draw_gradient_set(Pf, pat, Rb, NL, [1 1 1], [0.15 0.15 0.15], ...
    'e^{(e)} = (P_m^{(e)})^2  (porosity coefficient)', 'porosity_pattern_gradient.png', outdir);
disp('DONE: GPL_pattern_gradient.png + porosity_pattern_gradient.png');
