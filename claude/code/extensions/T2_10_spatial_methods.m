%% ========================================================================
%  T2_10_spatial_methods.m — RADIAL SPATIAL DISCRETIZATION COMPARISON
%  ------------------------------------------------------------------------
%  Compares DQM (Chebyshev and uniform node distributions), FDM (2nd order),
%  and FEM (linear and quadratic elements) for the 1-D radial transient
%  conduction benchmark (inner ramp + outer convection), against the exact
%  Bessel-series solution. DQM node sweep runs to N=201, FDM/FEM to N=321;
%  error is reported at t=10 s under the shared Newmark time march
%  (dt = 0.01 s). Finding: Chebyshev-node DQM stays well-conditioned across
%  the full sweep, while uniform-node DQM Runge-diverges (~1e47-1e52) beyond
%  N~31 from global-polynomial ill-conditioning. Output -> results_extensions\.
%  ========================================================================
...
%% ---- sweeps -------------------------------------------------------------
fprintf('===== SPATIAL METHOD CONVERGENCE incl. FEM (error at t=10 s) =====\n');
