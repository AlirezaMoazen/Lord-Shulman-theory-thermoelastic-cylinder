%% ========================================================================
%  claude_T2_8_spatial_methods.m — SPATIAL DISCRETIZATION COMPARISON (T2.8)
%  ------------------------------------------------------------------------
%  Revision of claude_T2_7_spatial_methods.m (T2_7 kept frozen). Author
%  request: (1) drop the "N" in the x-axis label (just "number of radial
%  points"); (2) add more DQM(Chebyshev)/FEM(quadratic) nodes. DQM-Chebyshev
%  already NaNs for N>=161 (Runge/ill-conditioning breakdown, T2.2 finding)
%  so pushing its ceiling higher adds no new valid data — instead the gap
%  between the last good point (121) and the first failure (161) is
%  densified (131/141/151) to pin down exactly where it breaks. FEM
%  (quadratic) previously stopped at N=161 while FDM/FEM(linear) run to
%  N=321 — extended to N=321 (odd, so (N-1)/2 elements stays integer) to
%  match that range, since quadratic FEM does not ill-condition the way
%  global-polynomial DQM does.
%  Everything else unchanged: same exact Bessel-series reference, same
%  Newmark time march (dt = 0.01 s) so the measured error at t = 10 s is
%  the SPATIAL error of each method. Output goes to results_extensions\.
%  ========================================================================
...
%% ---- sweeps -------------------------------------------------------------
fprintf('===== T2.8: SPATIAL METHOD CONVERGENCE incl. FEM (error at t=10 s) =====\n');
Ns_dqm = [7 9 11 15 21 31 41 61 81 121 131 141 151 161 201];
err_dqm = nan(size(Ns_dqm)); err_dqu = err_dqm;
for q = 1:numel(Ns_dqm)
    N = Ns_dqm(q);
    % Global-polynomial DQM differentiation matrices become ill-conditioned
    % at high N (esp. uniform nodes, Runge-type blow-up); guard with
    % try/catch so one bad N doesn't abort the whole sweep — a failed/NaN
    % point is itself the informative result (method breaks down there).
    try
...
Ns_fe2 = [11 21 41 81 161 321];               % quadratic (odd N, (N-1)/2 elems)
...
% "first edition" palette (MATLAB default color order): blue/red/yellow/purple/green
...
xlabel('number of radial points'); ylabel('max error at t = 10 s (K)');
legend('DQM (Chebyshev)','DQM (uniform)','FDM (2nd order)', ...
       'FEM (linear)','FEM (quadratic)','slope -2','Location','northeast');
grid on; box on;
% cap the y-axis: uniform-node DQM diverges to ~1e47-1e52 beyond N=31
% (global-polynomial ill-conditioning), which would otherwise flatten
% every other well-behaved curve into an unreadable line at the bottom.
ylim([1e-4 3e-1]);
