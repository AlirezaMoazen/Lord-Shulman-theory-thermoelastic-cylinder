%% ========================================================================
%  claude_T4_axial_methods.m — AXIAL SPATIAL DISCRETIZATION COMPARISON (T4)
%  ------------------------------------------------------------------------
%  Axial-direction companion to claude_T2_7_spatial_methods.m (which is the
...
%% ---- sweeps -------------------------------------------------------------
fprintf('===== T4: AXIAL SPATIAL METHOD CONVERGENCE incl. FEM (error at t=10 s) =====\n');
...
xlabel('number of axial points N'); ylabel('max error at t = 10 s (K)');
legend('DQM (Chebyshev)','DQM (uniform)','FDM (2nd order)', ...
       'FEM (linear)','FEM (quadratic)','slope -2','Location','northeast');
grid on; box on;
% axial problem resolves to lower error than the radial T2 case (temporal
% floor ~1e-4 here), so the y-axis extends down to 1e-5 to reveal the DQM /
% quadratic-FEM convergence instead of clipping it. Top still capped so the
% divergent uniform-DQM branch (~1e50) doesn't flatten the readable curves.
ylim([1e-5 3e-1]);
