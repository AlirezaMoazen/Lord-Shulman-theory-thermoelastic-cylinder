%% catalog_relax_finer_N35_R2.m — C_relaxation 4-panel figure at N_r=35
%  Reuses the 4-panel format and dimensionless convention from
%  catalog_R6.m's 'C_relaxation' study (T*(Fo), U*(Fo), T*(xi),
%  Sigma_thth(xi)) but reads the finer-mesh (N_r=35, N_z=11, total_time=6000,
%  dt=1) diagnostic data in param_studies_relax_finer\ instead of the locked
%  N_r=15 param_studies_ch4\ campaign. Purpose: check how much the N_r=15
%  negative-T* dip shrinks at N_r=35 for the three LS cases (Fourier's small
%  residual dip is real coupled-physics, not a discretization artifact).
%  Not part of the official campaign; output kept in its own folder so it
%  never touches figures_ch4_*.
