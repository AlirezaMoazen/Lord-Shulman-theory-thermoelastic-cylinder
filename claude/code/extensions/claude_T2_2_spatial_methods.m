%% ========================================================================
%  claude_T2_2_spatial_methods.m — SPATIAL DISCRETIZATION COMPARISON (T2.2)
%  ------------------------------------------------------------------------
%  Revision of claude_T2_1_spatial_methods.m (T2_1 kept frozen). Change:
%    + DQM node sweep extended from [7 9 11 15 21 31] to
%      [7 9 11 15 21 31 41 61 81 121 161 201] so the DQM curves span a
%      comparable N-range to FDM/FEM (author request: DQM was stopping at
%      N~20-31 while FDM/FEM run out to N=321, making it hard to see
%      whether DQM keeps converging or degrades at higher N).
%  Everything else is unchanged: same exact Bessel-series reference,
...
%  revision's output supersedes T2_1's on the same filenames.
%  ========================================================================
...
%% ---- sweeps -------------------------------------------------------------
fprintf('===== T2.2: SPATIAL METHOD CONVERGENCE incl. FEM (error at t=10 s) =====\n');
...
ylim([1e-4 3e-1]);
text(35, 1.6e-4, 'DQM (uniform) diverges for N \geq 41', 'FontSize',8,'Color',[0.85 0 0.85]);
