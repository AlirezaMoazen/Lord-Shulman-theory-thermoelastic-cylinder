%% ========================================================================
%  claude_T2_4_spatial_methods.m — SPATIAL DISCRETIZATION COMPARISON (T2.4)
%  ------------------------------------------------------------------------
%  Revision of claude_T2_3_spatial_methods.m (T2_3 kept frozen). Change
%  (author request, figure styling only — no numeric change):
%    + line colors switched back to the "first edition" palette (blue /
%      red / yellow / purple / green — MATLAB's default color order) that
%      the original T2 figure used, replacing T2_1..T2_3's blue/magenta/
%      red/green/black scheme.
%    + markers changed to hollow (no MarkerFaceColor) instead of solid-filled.
%  Everything else is unchanged: same DQM node sweep extended to N=201,
%  same exact Bessel-series reference, same Newmark time march (dt = 0.01 s)
%  so the measured error at t = 10 s is the SPATIAL error of each method.
%  Output goes to results_extensions\ (method/theory comparisons are kept
%  separate from the parametric campaign for the results chapter); this
%  revision's output supersedes T2_3's on the same filenames.
%  ========================================================================
...
%% ---- sweeps -------------------------------------------------------------
fprintf('===== T2.4: SPATIAL METHOD CONVERGENCE incl. FEM (error at t=10 s) =====\n');
...
loglog(Ns_dqm, err_dqm, 'o-', 'Color',cBlue,'LineWidth',1.6); hold on;
loglog(Ns_dqm, err_dqu, 's-', 'Color',cRed, 'LineWidth',1.3);
loglog(Ns_fdm, err_fdm, '^-', 'Color',cYel, 'LineWidth',1.6);
loglog(Ns_fe1, err_fe1, 'd-', 'Color',cPur, 'LineWidth',1.6);
loglog(Ns_fe2, err_fe2, 'v-', 'Color',cGrn, 'LineWidth',1.6);
