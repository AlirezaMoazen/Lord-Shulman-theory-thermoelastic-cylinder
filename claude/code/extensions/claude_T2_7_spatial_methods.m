%% ========================================================================
%  claude_T2_7_spatial_methods.m — SPATIAL DISCRETIZATION COMPARISON (T2.7)
%  ------------------------------------------------------------------------
%  Revision of claude_T2_6_spatial_methods.m (T2_6 kept frozen). Change
%  (author request, figure styling only — no numeric change): match the
%  same display standard used by the other Chapter 4 figures (e.g.
%  claude_catalog_bestworst.m) — solid colored line through each series,
%  marker with a WHITE face (not hollow/transparent, not solid-colored) so
%  the connecting line is masked inside the marker glyph but stays visible
%  between markers, matching the "line disappears inside the shape but
%  exists between shapes" look the author asked for.
...
%  revision's output supersedes T2_6's on the same filenames.
%  ========================================================================
...
%% ---- sweeps -------------------------------------------------------------
fprintf('===== T2.7: SPATIAL METHOD CONVERGENCE incl. FEM (error at t=10 s) =====\n');
