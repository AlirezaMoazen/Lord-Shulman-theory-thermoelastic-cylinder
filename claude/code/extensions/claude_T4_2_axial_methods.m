%% ========================================================================
%  claude_T4_2_axial_methods.m — AXIAL SPATIAL DISCRETIZATION COMPARISON (T4.2)
%  ------------------------------------------------------------------------
%  Revision of claude_T4_1_axial_methods.m (T4_1 kept frozen). Author
%  request (mirrors T2.8): (1) drop the "N" in the x-axis label; (2) add
%  more DQM(Chebyshev)/FEM(quadratic) nodes. DQM-Chebyshev already NaNs for
%  N>=161 (same Runge/ill-conditioning breakdown as the radial T2 case) so
%  the gap between the last good point (121) and the first failure (161)
%  is densified (131/141/151) to pin down where it breaks. FEM (quadratic)
%  extended from N=161 to N=321 to match FDM/FEM(linear)'s range.
%  ------------------------------------------------------------------------
%  Axial-direction companion to claude_T2_8_spatial_methods.m (which is the
...
%% ---- sweeps -------------------------------------------------------------
fprintf('===== T4.2: AXIAL SPATIAL METHOD CONVERGENCE incl. FEM (error at t=10 s) =====\n');
