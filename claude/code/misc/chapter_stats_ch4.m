%% ========================================================================
%  claude_chapter_stats_ch4.m — key dimensionless numbers per campaign case
%  ------------------------------------------------------------------------
%  REVISION of claude_chapter_stats.m: reads the NEW-geometry campaign folder
%  (param_studies_ch4, produced by run_ch4_campaign.ps1 on the R7/R8 solver)
%  instead of the old param_studies folder, and uses the NEW reference
%  geometry/material for normalization. The old script pointed at the stale
%  old-geometry runs (NL=5, R_o=0.2), which is why chapter_stats.csv disagreed
%  with the Chapter-4 R4.4 numbers. Output overwrites thesis_chapter/chapter_stats.csv.
%    Tmax_star / Fo_at_Tmax : peak mid-point temperature (overshoot check)
%    Tend_star / Tout_end_star : mid-point & outer-surface temperature at t_end
%    umax_star / uend_star  : peak & final mid-point radial displacement
%    Stt_in/out/min/max_end_star : final hoop stresses
%  ========================================================================
clearvars; clc;

% ---- reference constants (NEW reference: UD-GPL W=0.3%, UD-porosity em3=0.8604) ----
...
Wg=0.003; em3=0.8604;                          % <-- NEW reference (was 0.04 / 0.8980)
...
R_o=1.5; hthick=0.5; dT=300; T_inf=300;        % <-- NEW geometry (was R_o=0.2 / h=0.1)
Fo  = @(t) alpha_diff*t/hthick^2;              % <-- Fo on thickness (matches R4.4)
Tst = @(T) (T - T_inf)/dT;
ust = @(u) u*(lam_+2*mu_)/(beta_ref*dT*hthick);
sst = @(s) s/(beta_ref*dT);

% ---- sweep all case files in the NEW campaign folder ---------------------
CLROOT = 'c:/Users/InfosaicUser/Desktop/MSc/Lord-Shulman-theory-thermoelastic-cylinder/claude';
srcdir = fullfile(CLROOT,'param_studies_ch4');  % <-- NEW folder (absolute; was param_studies)
outdir = fullfile(CLROOT,'thesis_chapter');  if ~exist(outdir,'dir'), mkdir(outdir); end
L = dir(fullfile(srcdir,'*.mat'));
fprintf('DEBUG: NFILES=%d in %s (cwd=%s)\n', numel(L), srcdir, pwd);
skip = {};                                     % all files here are real cases
rows = {};
for i = 1:numel(L)
    [~,cname] = fileparts(L(i).name);
    if any(strcmp(cname, skip)), continue; end
    D = load(fullfile(srcdir, L(i).name));
...
% quick sanity print
ib = find(strcmp(T.case_,'BASE'),1);
if ~isempty(ib), fprintf('SANITY: BASE Tmax_star = %.4f (R4.4 expects ~0.926)\n', T.Tmax_star(ib)); end
