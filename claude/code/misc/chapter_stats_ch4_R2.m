%% ========================================================================
%  chapter_stats_ch4_R2.m — key dimensionless numbers per campaign case
%  ------------------------------------------------------------------------
%  Reads the campaign folder (param_studies_ch4, produced by
%  run_ch4_campaign.ps1) and normalizes each case's results against the
%  reference geometry/material. Output overwrites thesis_chapter/chapter_stats.csv.
...
% ---- reference constants (UD-GPL W=0.3%, UD-porosity em3=0.8604) ----
...
R_o=1.5; hthick=0.5; dT=300; T_inf=300;
Fo  = @(t) alpha_diff*t/hthick^2;              % Fourier number based on layer thickness
Tst = @(T) (T - T_inf)/dT;
ust = @(u) u*(lam_+2*mu_)/(beta_ref*dT*hthick);
sst = @(s) s/(beta_ref*dT);

% ---- sweep all case files in the campaign folder --------------------------
CLROOT = 'c:/Users/InfosaicUser/Desktop/MSc/Lord-Shulman-theory-thermoelastic-cylinder/claude';
srcdir = fullfile(CLROOT,'param_studies_ch4');  % campaign folder (absolute path)
...
if ~isempty(ib), fprintf('SANITY: BASE Tmax_star = %.4f (expected ~0.926)\n', T.Tmax_star(ib)); end
