%% ========================================================================
%  LSTE_solver_R10.m  —  DYNAMIC THERMOELASTIC ANALYSIS (MULTI-THEORY)
%  Multilayer porous GPL-reinforced cylinder, layerwise DQM + Newmark (beta)
%  ------------------------------------------------------------------------
%  REVISION R10 (author request, 2026-09-02): completes the porosity
%  REPORTING values e1, e2 (O/X patterns) that R9 left as NaN with a
%  comment "descriptive only (not used)". They are STILL not used by the
%  physics -- E/k/c/rho are computed pointwise per layer via Pf=Pm^2, as
%  in R9, completely unaffected by this change. This only fills in the
%  two scalar cross-check values, matching what e4/e5 already had.
%  Unlike e4/e5 (which have the closed form e4=(pi/2)*em4^2, derived from
%  Porosity_patterns_doc.docx's "A" self-consistency trick and verified),
%  e1/e2 have NO closed form -- integrating the document's own pointwise
%  relation 1-em1*cos(theta) = sqrt(1-e1*cos(theta)) is a non-elementary
%  (elliptic-type) integral. So e1/e2 are solved NUMERICALLY here, from
%  the same "equal-average-mass" integral used to derive em1/em2/em4/em5
%  in the first place:  integral( sqrt(1 - e*pattern(zeta)) , -1/2, 1/2 ) = em3.
%  Verified against Porosity_patterns_doc.docx's own table (matches to
%  the table's stated precision) and against Main_Dyn_R4.m's own
%  hardcoded e1=0.1 for em3=0.9675 (exact match). Everything else is
%  byte-identical to R9 (physics/performance unchanged).
%  ------------------------------------------------------------------------
...
% (R10) e1, e2: no closed form -- solved numerically (see header). Still
