...
N_r = 9;                    % number of radial points per layer (match dynamic R5 grid)
N_z = 11;                   % number of axial points (match dynamic R5 grid)
...
%% ========================= 3. PRE-COMPUTATION FOR V AND A POROSITY PATTERNS =========================
l_total = R_o - R_i;
...
%% ========================= 4. COMPUTATION OF EFFECTIVE PROPERTIES =========================
E_node = cell(NL, 1);
nu_node = cell(NL, 1);
rho_node = cell(NL, 1);
c_node = cell(NL, 1);
...
for e = 1:NL
    ...
    if V_GPL > 1/p   % formula valid only for V_GPL > 1/p (else negative/complex)
...
%  Applied after internal mechanical assembly, before boundary conditions
%% ============================================================
...
% save results for cross-validation against the dynamic solver
save('Results_Static.mat','T_layer','U_layer','W_layer','r_nodes','z_nodes', ...
     'NL','N_r','N_z','P_i','T_i_val','T_inf','h_c');
disp('Saved Results_Static.mat');
