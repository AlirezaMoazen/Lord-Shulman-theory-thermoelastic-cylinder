...
examined independently in each discretization direction — the radial nodes per
layer N_r, the axial nodes N_z, the number of layers N_L, and the time step Δt.
The convergence measure is the dimensionless hoop stress, reported as both the
inner-surface value and the relative L₂ error of the whole mid-plane radial
profile against the finest mesh. For each spatial direction the profile overlays
are shown as a figure and the error and cost are collected in a table.

### 4-3-1. Radial nodes (N_r)

**4-1**

![Figure 4-1: Radial-node convergence — hoop-stress and temperature profiles for N_r = 7…15 [N_z=11, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/conv_Nr.png){width=6in}

Figure 4-1 shows the hoop-stress and temperature profiles for N_r = 7 … 15
(with N_z = 11, N_L = 7); the curves are visually indistinguishable. As Table 4-2
shows, the inner-surface hoop stress stays within 0.09 % and the L₂ profile error
below 0.29 % over the whole range, so the field quantities are converged from as
few as seven radial points — the hallmark of the spectral (Chebyshev) differential
quadrature discretization. N_r = 15 is nevertheless adopted for the production
runs because it renders the sharp, moving second-sound front in the temperature
profiles cleanly, without the localized oscillation a coarser mesh leaves when the
front sits between two nodes; this does not change any integral quantity.

**Table 4-2: Radial-node convergence (N_z = 11, N_L = 7).**

| N_r | Σθθ (inner) | L₂ error (%) | N_dof |
|---|---|---|---|
| 7 | 1.5251 | 0.28 | 1617 |
| 9 | 1.5256 | 0.23 | 2079 |
| 11 | 1.5256 | 0.24 | 2541 |
...
### 4-3-2. Axial nodes (N_z)

**4-2**

![Figure 4-2: Axial-node convergence — profiles for N_z = 5…15 [N_r=15, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/conv_Nz.png){width=6in}

The axial direction (Figure 4-2) governs the mesh. With only five axial points the
inner-surface hoop stress is in error by 9.4 %, because the simply-supported end
layer is left unresolved; the error then settles below about 1 % from nine points
onward (Table 4-3). The convergence is non-monotonic — the oscillatory envelope
typical of spectral methods — but is clearly settled by nine points, and eleven
points give an L₂ error of 0.16 %. Eleven axial points are adopted. This direction
is also the most expensive: the single-core solve time grows roughly as N_z^2.6
(Table 4-3), because each added axial point multiplies the entire through-thickness
system. The axial direction is therefore the binding one — both the least accurate
per node and the steepest in cost.

**Table 4-3: Axial-node convergence and cost (N_r = 15, N_L = 7).**

| N_z | Σθθ (inner) | L₂ error (%) | solve time (s) |
|---|---|---|---|
| 5 | 1.3844 | 3.92 | 90 |
| 7 | 1.5333 | 1.07 | 198 |
| 9 | 1.5434 | 0.23 | 349 |
...
### 4-3-3. Number of layers (N_L)

**4-3**

![Figure 4-3: Layer convergence — profiles for N_L = 3…15 [N_r=15, N_z=11, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/conv_NL.png){width=6in}

Figure 4-3 shows the profiles for N_L = 3 … 15 (N_r = 15, N_z = 11). The
inner-surface hoop stress varies by less than 0.05 % and the L₂ error stays below
0.26 % (Table 4-4); the field quantities are essentially layer-independent from
three layers. The only effect of more layers is a finer staircase in the
hoop-stress profile at the interfaces — the natural signature of the layerwise
property model. Seven layers are adopted, matching the seven-layer graded build of
the reference cylinder.

**Table 4-4: Layer convergence (N_r = 15, N_z = 11).**

| N_L | Σθθ (inner) | L₂ error (%) | N_dof |
|---|---|---|---|
| 3 | 1.5261 | 0.26 | 1485 |
| 5 | 1.5257 | 0.21 | 2475 |
| 7 | 1.5264 | 0.16 | 3465 |
...
The time step is converged for Δt ≤ 2 s; at Δt = 5 s a spurious 3 % overshoot
appears in the peak temperature as the Newmark integrator under-resolves the sharp
thermal front (Table 4-5). Δt = 1 s is adopted, which resolves the shock with a
comfortable margin.

**Table 4-5: Time-step convergence (N_r = 15, N_z = 11, N_L = 7).**

| Δt (s) | time steps | L₂ error (%) | note |
|---|---|---|---|
| 0.5 | 6000 | ref | — |
| 1 | 3000 | 0.17 | adopted |
| 2 | 1500 | 0.25 | — |
| 5 | 600 | 0.64 | 3 % peak overshoot |

Overall, the adopted production mesh (N_r = 15, N_z = 11, N_L = 7, Δt = 1 s)
delivers the field quantities to better than 0.2 %: the radial and layer directions
are effectively free, while the axial direction is the binding and most expensive
one. The superiority of the differential-quadrature discretization over the finite
difference and finite element methods, in points required per direction, is taken
up separately in Section 4-19.
...
behavior of the coupled Lord-Shulman theory is compared qualitatively with the results of
Bagri and Eslami (2007, reference [7]); both wave speeds (elastic and thermal)
and the reflection times within the cylinder wall are correctly reproduced. Taken together,
...
The two checks that compare against published or exact references are shown
below — the mechanical response against Malekzadeh & Heydarpour and ANSYS, the
transient conduction against the exact Bessel-series solution — with the present curves
coinciding with the references in both cases.

**4-4**

![Figure 4-4: Verification (mechanical) — inner-surface dimensionless radial displacement of the present solver against Malekzadeh & Heydarpour (2012) and an independent ANSYS solution [thick cylinder under internal pressure step]](figures_ch4_color/bench1_U.png){width=6in}

**4-5**

![Figure 4-5: Verification (thermal) — transient temperature profiles of the present DQM+Newmark solver against the exact Bessel-series solution at t = 2, 10 and 40 s](figures_ch4_color/bench2_T_profiles.png){width=6in}
...
their reference values. Figure 4-6 shows the time histories of the dimensionless
...
![Figure 4-6: Effect of the GPL distribution pattern (UD, O, X, V, A) on the mid-point T* and U* histories and the final T* and Σθθ profiles [R_i=1, R_o=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/A_GPL_patterns.png){width=6in}
...
(UD) and the remaining parameters are held at their reference values. Figure 4-7
...
![Figure 4-7: Effect of the porosity distribution pattern (UD, O, X, V, A), GPL uniform [R_i=1, R_o=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/B_porosity_patterns.png){width=6in}
...
![Figure 4-8: Effect of the porosity level e_m3 = 0.9675 / 0.8604 / 0.7776 [R_o/R_i=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, UD, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/E_porosity_level.png){width=6in}

Finally, the effect of the overall porosity level is shown in Figure 4-8 for the
...
![Figure 4-9: Effect of the GPL weight fraction (low range) W = 0.1 / 0.3 / 0.5 / 0.9 / 1.5 % [R_o/R_i=1.5, L=2.1 m, N_L=7, UD, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/D_wt_low.png){width=6in}

The percolation character of the thermal-conductivity model (Chapter 3) makes the
GPL weight fraction the strongest material lever of the problem. Figure 4-9 scans
...
![Figure 4-10: Effect of the GPL weight fraction (high range) W = 1 / 2 / 4 / 8 % [R_o/R_i=1.5, L=2.1 m, N_L=7, UD, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/D2_wt_high.png){width=6in}

The higher range W = 1, 2, 4 and 8 % is shown in Figure 4-10. Here the thermal
...
![Figure 4-11: Effect of the platelet length-to-width aspect ratio a/b = 1.0 / 1.67 / 2.67 [R_o/R_i=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/O_aspect_ab.png){width=6in}

The effect of the length-to-width ratio is shown in Figure 4-11. As a/b increases
...
![Figure 4-12: Effect of the platelet width-to-thickness aspect ratio b/t = 500 / 1000 / 2000 [R_o/R_i=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/P_aspect_bt.png){width=6in}

The width-to-thickness ratio (Figure 4-12) is even weaker at this reference: as b/t
...
![Figure 4-13: Effect of the relaxation time — Fourier (τ*=0) and Lord–Shulman τ* = 0.04 / 0.15 / 0.44 [R_o/R_i=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, P_i=50 MPa, simply supported]](figures_ch4_color/C_relaxation.png){width=6in}

Figure 4-13 compares Fourier conduction (τ* = 0) with four dimensionless
...
![Figure 4-14: Effect of the end supports — simply supported (S-S), mixed (S-C) and clamped (C-C) [R_o/R_i=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa]](figures_ch4_color/F_end_BC.png){width=6in}
...
at both ends (C-C). As shown in Figure 4-14, the temperature field is nearly
...
(reference) and 100 MPa, covering low to very high pressures. As shown in Figure 4-15, the temperature field is practically independent of the internal pressure,
...
![Figure 4-15: Effect of the internal pressure P_i = 0 / 10 / 50 / 100 MPa [R_o/R_i=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, simply supported]](figures_ch4_color/G_pressure.png){width=6in}
...
![Figure 4-16: Effect of the cylinder length L = 1 / 2.1 / 5 / 10 m [R_i=1, R_o=1.5, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/Q_length.png){width=6in}

To examine the limiting behavior of a long cylinder, the length is increased from
1 m through the reference 2.1 m to 5 and 10 m. As shown in Figure 4-16, the peak
...
of every combination are shown as a 5×5 matrix in Figure 4-17.

**4-17**

![Figure 4-17: Full 25-case GPL × porosity matrix — through-wall dimensionless temperature T*(ξ) for every pattern pair (rows: GPL pattern; columns: porosity pattern) [reference: W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/matrix25_Tstar.png){width=6in}
...
![Figure 4-18: Full 25-case GPL × porosity matrix — hoop-stress profile Σθθ(ξ) for every pattern pair (rows: GPL pattern; columns: porosity pattern) [reference: W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/matrix25_Sigma_thth.png){width=6in}
...
While the 5×5 matrix gives the complete overview at a glance, the same twenty-five combinations are shown one porosity background at a time in Figures 4-19 to 4-23 — for each porosity pattern (UD, O, X, V and A) a single figure that overlays the five GPL patterns. The GPL ranking at a fixed porosity then reads directly: against the porosity-A barrier (Figure 4-23), for example, GPL-V keeps the outer surface coolest while GPL-A and GPL-X run hottest, whereas at uniform porosity (Figure 4-19) the spread between the patterns is much narrower.

**4-19**

![Figure 4-19: GPL pattern comparison (UD, O, X, V, A) at uniform (UD) porosity [R_i=1, R_o=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, UD porosity, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/gplpor_porUD.png){width=6in}

**4-20**

![Figure 4-20: GPL pattern comparison (UD, O, X, V, A) at O-type porosity [R_i=1, R_o=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, O porosity, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/gplpor_porO.png){width=6in}

**4-21**

![Figure 4-21: GPL pattern comparison (UD, O, X, V, A) at X-type porosity [R_i=1, R_o=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, X porosity, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/gplpor_porX.png){width=6in}

**4-22**

![Figure 4-22: GPL pattern comparison (UD, O, X, V, A) at V-type porosity [R_i=1, R_o=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, V porosity, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/gplpor_porV.png){width=6in}

**4-23**

![Figure 4-23: GPL pattern comparison (UD, O, X, V, A) at A-type porosity [R_i=1, R_o=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, A porosity, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/gplpor_porA.png){width=6in}

Read side by side, the five per-porosity figures separate the two mechanisms cleanly. In every background the ordering of the GPL patterns by outer-surface temperature is the same — V the coolest, then O, then UD and X, with A the hottest — which shows that the graphene pattern and the porosity pattern act through largely independent channels: the graphene sets how quickly heat is conducted across the thickness, while the porosity sets how much of it is blocked near the outer wall. What changes from one figure to the next is the spread between the five curves. Against the uniform (UD) or inner-concentrated (V) porosities the curves stay bunched, because those pore distributions do little to impede the outward heat flow, so the choice of graphene pattern matters least there. Against the outer-concentrated porosity-A the curves fan apart and every one of them falls to a very low outer-surface temperature, because the outer pore barrier and a conductive inner graphene layer reinforce one another — this is the background on which the graphene pattern pays off most. The hoop-stress profiles follow the same logic: the gentlest interface staircases belong to the GPL-V backgrounds, where the stiff conductive phase sits exactly where the temperature gradient is steepest, while the steepest jumps appear for the inner-concentrated porosity-V, which piles both the thermal load and the compliant, pore-rich material at the heated inner surface.
...
![Figure 4-26: Effect of the outer-surface convection coefficient h_c = 10 / 100 / 1000 W/m²K [R_o/R_i=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/J_convection.png){width=6in}
...
![Figure 4-27: Effect of the wall thickness, radius ratio R_o/R_i = 1.25 / 1.5 / 2.0 (inner radius fixed at 1 m) [L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/K_thickness.png){width=6in}
...
![Figure 4-28: Response to a Gaussian thermal shock — Lord–Shulman versus Fourier [R_o/R_i=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, P_i=50 MPa, simply supported]](figures_ch4_color/M_gauss_shock.png){width=6in}
...
behind. The Lord-Shulman solution shows a sharp
...
**Table 4-7: Comparison of time-integration methods on the same spatial system.**
...
![Figure 4-29: Spatial-discretization convergence on the transient-conduction benchmark: maximum error versus number of radial nodes N for DQM (Chebyshev and uniform), 2nd-order FDM, and linear/quadratic FEM; the dotted line marks slope -2 (second-order convergence)](figures_ch4_color/spatial_convergence.png){width=6in}
## 4-20. Comparison of the generalized thermoelasticity theories

**4-30**

![Figure 4-30: Comparison of the generalized thermoelasticity theories — Fourier, Lord–Shulman, dual-phase-lag and Green–Naghdi III [R_o/R_i=1.5, L=2.1 m, N_L=7, W_GPL=0.3%, e_m3=0.8604, τ*=0.15, P_i=50 MPa, simply supported]](figures_ch4_color/T3_theories.png){width=6in}
